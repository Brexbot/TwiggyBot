...
const preferred_letter: Record<string, string> = {
    STR:'S', DEX:'D', CON:'C', INT:'I', WIS:'W', CHR:'T'
};
...
export class Character {
    ...
    constructor(data: string)   {
        ...
        var adjective: string = get_seeded_random_element(adjectives, this.rng);
        var noun: string = get_seeded_random_element(nouns, this.rng);
        ...
        for (const [key, value] of Object.entries(preferred_letter)) {
            ...
            let char = preferred_letter[key];
            let total_rolls = 1 + Math.max(ustr.split(char).length + twos, 2);
            this.stats[key] = roll_seeded_dy_x_times_pick_z(6, total_rolls, 3, this.rng);
        }

        // Now add the species modifier.
        for (var i = 0; i < this.character_specie.stat_bonuses.length; i++)    {
    ...
    toString()  {
        ...
        ostr += `  ${this.stats['STR'].toString().padStart(2, ' ')} |  ${this.stats['DEX'].toString().padStart(2, ' ')} |  ${this.stats['CON'].toString().padStart(2, ' ')} |  ${this.stats['INT'].toString().padStart(2, ' ')} |  ${this.stats['WIS'].toString().padStart(2, ' ')} |  ${this.stats['CHR'].toString().padStart(2, ' ')} \n`
        ostr += "\`\`\`"
        return ostr;
    }
}
